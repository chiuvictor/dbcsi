...
Prompt This script assumes that you have a valid license for Diagnostics Pack.  Do not run this script if you do not.
Prompt If you do not have a Diagnostic Pack license, ask your AWS contact for a version of this script that is designed to work with STATSPACK
REM  PLEASE READ THE ABOVE LINE  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Prompt
Prompt NOTICE: If using Multi-tenant, please connect only to the Container database.  The script will not collect proper data if you connect directly to a PDB in a multi-tenant architecture
Prompt
...
define AWR_MINER_VER = 4.0.11.aws.21
...
Begin
    ...
    if IS_OK = 'NO'  then
        dbms_output.put_line('This script is expecting to be able to use the DBA_HIST tables which require the Diagnostics Pack.');
        ...
        dbms_output.put_line('The script will now exit.');
...
REM alter session set container=cdb$root;
...
REM if you want to run for a specific range of snapshots, edit/uncomment the following line.  
...
set termout on
set serveroutput on
prompt Confirming that we are not connected to a PDB.
prompt
...
Begin
    ...
    if :DB_VERSION_1 >= 12.0 AND IS_CDB = 'YES' AND CON_ID != 1 then
        dbms_output.put_line('This is a container databse and looks like we are connected to the PDB not the CDB.');
        ...
        dbms_output.put_line('This is a container databse and we are connected to the CDB ROOT.');        
...
/
set termout off
select :CDB_OR_DBA_COL_1, :DISK_SIZE_12C_WHERE_1, :DISK_SIZE_12C_1, :DISK_SIZE_12C_TABLE_1, :CDB_PDB_VIEW_1, :CDB_PDB_VIEW2_1, :CDB_PDB_EXPR_1 from dual;
...
BEGIN
    ...
    FOR c3a IN (SELECT count(distinct s.instance_number) instances
			     FROM dba_hist_database_instance i,dba_hist_snapshot s
				WHERE i.dbid = s.dbid
				  and i.dbid = &DBID
	...
	FOR c6 IN (select /*+ inline */ round(sum(bytes)/1024/1024/1024) Total_DB_size_in_Gb from (select /*+ inline */ bytes from &CDB_OR_DBA_COL~_data_files union all select /*+ inline */ bytes from &CDB_OR_DBA_COL~_temp_files))
    loop
	...
	FOR c5 IN (SELECT sys_context('USERENV', 'MODULE') module, &CDB_PDB_EXPR con_name FROM DUAL)
    loop
    ...
    FOR c14 in (select name, value from v$parameter
      where name in ('statistics_level','timed_statistics','control_management_pack_access')
        order by name)
    loop
...
select name,detected_usages, total_samples, currently_used,aux_count,
